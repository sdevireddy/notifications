...
const quoteColumnsConfig = {
    ...
    quoteStage: {
        label: "Quote Stage",
        render: ({ row }) => row.original.quoteStage || "-",
    },
    grandTotal: {
        label: "GrandTotal",
    ...
    tax: {
        label: "Tax",
        render: ({ row }) => row.original.tax || "-",
    },
    discount: {
        label: "Discount",
        render: ({ row }) => row.original.discount || "-",
    },
};
...
export default function QuotePage() {
    ...
    const [quoteData, refetchData, loading] = useFetchData(apiSummary.crm.getQuote, currentPage, recordsPerPage);
    const [visibleColumns, setVisibleColumns] = useState(availableAccountColumns);
    const [showColumnSelector, setShowColumnSelector] = useState(false);
    const [actionOpen, setActionOpen] = useState(false);
    useEffect(() => {
        setQuotes(quoteData.data);
        setFilteredQuotes(quoteData.data);
        setTotalRecords(quoteData.totalRecords);
        setCurrentPage(1);
    }, [quoteData]);
    ...
    useEffect(() => {
        ...
        setFilteredQuotes(
            quotes.filter(
                (quote) =>
                    quote.accountName?.toLowerCase().includes(term) ||
                    quote.phone?.toLowerCase().includes(term) ||
                    quote.accountOwner?.toLowerCase().includes(term) ||
                    quote.website?.toLowerCase().includes(term),
            ),
        );
        setCurrentPage(1);
    }, [searchTerm]);
    ...
    const handleSelectAll = () => {
        selectMultipleAccount(selectMultipleAccount.length === quotes.length ? [] : quotes);
    };
    ...
    const columns = useMemo(() => {
        const dynamicCols = Object.entries(quoteColumnsConfig)
        ...
        <div className="flex-1 bg-white">
            <div className="flex items-center justify-between px-6 py-2">
                <div className="flex items-center gap-4">
                    <h1 className="text-xl font-semibold text-gray-900">Qoutes</h1>
                    <BreadCrumb />
                </div>
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        >
                            {Object.keys(quoteColumnsConfig).map((key) => (
                                <DropdownMenuItem
                                    key={key}
                                    onSelect={(e) => e.preventDefault()}
                                    className="flex items-center justify-between"
                                >
                                    <span className="text-sm">{quoteColumnsConfig[key].label}</span>
                                    <Checkbox
            ...
            <div className="flex items-center justify-between border-t bg-gray-50 px-6 py-2">
                <div className="text-sm text-gray-600">
                    Showing {indexOfFirstRecord + 1} to {Math.min(indexOfLastRecord, filteredQuotes.length)} of {totalRecord} results
                </div>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
