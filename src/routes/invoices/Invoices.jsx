...
const invoicesColumnsConfig = {
    subject: {
     ...
     invoiceDate: {
        label: "Invoice Date",
        render: ({ row }) => row.original.invoiceDate || "-",
    },
    status: {
        label: "Status",
    ...
    accountName: {
        label: "Account Name",
        render: ({ row }) => row.original.accountName || "-",
    },
     invoicesrOwner: {
        label: "Invoice Owner",
        render: ({ row }) => row.original.invoicesrOwner || "-",
    },
};
...
export default function InvoicePage() {
    ...
    const [invoicesData, refetchData, loading] = useFetchData(apiSummary.crm.getInvoices,currentPage,recordsPerPage);
    ...
    useEffect(() => {
        ...
        setTotalRecords(invoicesData.totalRecords);
        setCurrentPage(1);
    }, [invoicesData]);
    ...
    useEffect(() => {
        ...
        setFilteredInvoices(
            invoicesData.filter(
                (invoice) =>
                    invoice.accountName?.toLowerCase().includes(term) ||
                    invoice.phone?.toLowerCase().includes(term) ||
                    invoice.accountOwner?.toLowerCase().includes(term) ||
                    invoice.website?.toLowerCase().includes(term),
            ),
        );
        setCurrentPage(1);
    }, [searchTerm]);
    ...
    const handleSelectAll = () => {
        selectMultipleAccount(selectMultipleAccount.length === invoices.length ? [] : invoices);
    };
    ...
    const columns = useMemo(() => {
        const dynamicCols = Object.entries(invoicesColumnsConfig)
        ...
        <div className="flex-1 bg-white">
            <div className="flex items-center justify-between  px-6 py-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        >
                            {Object.keys(invoicesColumnsConfig).map((key) => (
                                <DropdownMenuItem
                                    key={key}
                                    onSelect={(e) => e.preventDefault()}
                                    className="flex items-center justify-between"
                                >
                                    <span className="text-sm">{invoicesColumnsConfig[key].label}</span>
                                    <Checkbox
                    ...
                    <DropdownMenu  open={actionOpen}
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="data-[highlighted]:bg-blue-100 data-[highlighted]:text-gray-900" onClick={()=>navigate('/import/invoices')}>
            ...
            <div className="flex items-center justify-between border-t bg-gray-50 px-6 py-2">
                <div className="text-sm text-gray-600">
                    Showing {indexOfFirstRecord + 1} to {Math.min(indexOfLastRecord, filteredInvoices.length)} of {totalRecord} results
                </div>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
