    ...
    <div className="relative bg-white p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
          ...
          <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={() => navigate("/LeadCreationForm")}>
            Add Lead
          </button>
          <button
          ...
          <tbody>
            {displayedLeads.map((lead, index) => (
              <tr key={lead.id} className={`hover:bg-gray-100 ${index % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
                ...
                <Link to={`/WhatsAppChat/${lead.id}`} state={{ lead }}  className="text-blue-500">
                  <FaWhatsapp className="text-green-600 cursor-pointer" />
                </Link>
               
                <Link to={`/callhistory/${lead.id}`} state={{ lead }}  className="text-blue-500">
                <FiPhone className="text-blue-500 cursor-pointer" />
                </Link>
                 
                  <Link to={`/emails/${lead.id}`} state={{ lead }}  className="text-blue-500">
