...
const vendorColumnsConfig = {
    vendorName: {
        label: "Vendor Name",
        render: ({ row }) => row.original.vendorName || "-",
    },
    email: {
        label: "Email",
    ...
    glAccount: {
        label: "GL Account",
        render: ({ row }) => row.original.glAccount || "-",
    },
    description: {
        label: "Description",
        render: ({ row }) => row.original.description || "-",
    },
     vendorOwner: {
        label: "vendorOwner",
        render: ({ row }) => row.original.vendorOwner || "-",
    },
};
...
export default function VendorPage() {
    ...
    const [vendorData, refetchData, loading] = useFetchData(apiSummary.crm.getVendor,currentPage,recordsPerPage);
    ...
    useEffect(() => {
        ...
        setTotalRecords(vendorData.totalRecords);
        setCurrentPage(1);
    }, [vendorData]);
        ...
        setFilteredVendors(
            vendors.filter(
                (vendor) =>
                    ...
                    vendor.accountOwner?.toLowerCase().includes(term) ||
    ...
    const handleSelectAll = () => {
        selectMultipleAccount(selectMultipleAccount.length === vendors.length ? [] : vendors);
    };
    ...
    const columns = useMemo(() => {
        const dynamicCols = Object.entries(vendorColumnsConfig)
        ...
        <div className="flex-1 bg-white">
            <div className="flex items-center justify-between  px-6 py-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        >
                            {Object.keys(vendorColumnsConfig).map((key) => (
                                <DropdownMenuItem
                                    key={key}
                                    onSelect={(e) => e.preventDefault()}
                                    className="flex items-center justify-between"
                                >
                                    <span className="text-sm">{vendorColumnsConfig[key].label}</span>
                                    <Checkbox
                    ...
                    <DropdownMenu  open={actionOpen}
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="data-[highlighted]:bg-blue-100 data-[highlighted]:text-gray-900" onClick={()=>navigate('/import/vendors')}>
            ...
            <div className="flex items-center justify-between border-t bg-gray-50 px-6 py-2">
                <div className="text-sm text-gray-600">
                    Showing {indexOfFirstRecord + 1} to {Math.min(indexOfLastRecord, filteredVendors.length)} of {totalRecord} results
                </div>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
