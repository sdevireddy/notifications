...
const productColumnsConfig = {
    productName: {
        label: "Product Name",
        render: ({ row }) => row.original.productName || "-",
    },
    productCode: {
        label: "Product Code",
        render: ({ row }) => row.original.productCode || "-",
    },
    status: {
        label: "Status",
    ...
    manufacturer: {
        label: "Manufacturer",
        render: ({ row }) => row.original.manufacturer || "-",
    },
    unitPrice: {
        label: "Unit Price",
        render: ({ row }) => row.original.unitPrice || "-",
    },
    productOwner: {
        label: "Product Owner",
        render: ({ row }) => row.original.productOwner || "-",
    },
     vender: {
        label: "Vender",
        render: ({ row }) => row.original.vender || "-",
    },
};
...
export default function ProductPage() {
    ...
    const [productData, refetchData, loading] = useFetchData(apiSummary.crm.getProduct,currentPage,recordsPerPage);
    ...
    useEffect(() => {
        ...
        setTotalRecords(productData.totalRecords);
        setCurrentPage(1);
    }, [productData]);
    ...
    useEffect(() => {
        ...
        setFilteredProduct(
            productData.filter(
                (product) =>
                    product.accountName?.toLowerCase().includes(term) ||
                    product.phone?.toLowerCase().includes(term) ||
                    product.accountOwner?.toLowerCase().includes(term) ||
                    product.website?.toLowerCase().includes(term),
            ),
        );
        setCurrentPage(1);
    }, [searchTerm]);
    ...
    const handleSelectAll = () => {
        selectMultipleAccount(selectMultipleAccount.length === products.length ? [] : products);
    };
    ...
    const columns = useMemo(() => {
        const dynamicCols = Object.entries(productColumnsConfig)
        ...
        <div className="flex-1 bg-white">
            <div className="flex items-center justify-between  px-6 py-2">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        >
                            {Object.keys(productColumnsConfig).map((key) => (
                                <DropdownMenuItem
                                    key={key}
                                    onSelect={(e) => e.preventDefault()}
                                    className="flex items-center justify-between"
                                >
                                    <span className="text-sm">{productColumnsConfig[key].label}</span>
                                    <Checkbox
                    ...
                    <DropdownMenu  open={actionOpen}
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="data-[highlighted]:bg-blue-100 data-[highlighted]:text-gray-900" onClick={()=>navigate('/import/product')}>
            ...
            <div className="flex items-center justify-between border-t bg-gray-50 px-6 py-2">
                <div className="text-sm text-gray-600">
                    Showing {indexOfFirstRecord + 1} to {Math.min(indexOfLastRecord, filteredProduct.length)} of {totalRecord} results
                </div>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
            ...
            <DeleteConfirmationDialog
                ...
                description="This product will be permanently deleted. Are you sure?"
                onConfirm={async () => {
                    setIsDeleting(true);
